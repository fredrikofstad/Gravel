...
namespace Gravel {
	...
	namespace Utilities {
		...
		static GLenum HazelFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			...
			GR_CORE_ASSERT(false, "change assert comments to optional");
			return 0;
		}
	...
	void OpenGLFrameBuffer::Recalculate()
	{
		...
		{
			GR_CORE_ASSERT(m_colorAttachments.size() <= 4, "size is more than 4");
	...
	{
		GR_CORE_ASSERT(attachmentIndex < m_colorAttachments.size(), "index out of bounds");
	...
	void OpenGLFrameBuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_colorAttachments[attachmentIndex], 0,
			Utilities::HazelFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
